...
<body>
    ...
    <div class="row mt-5 no-gutters">
        ...
        <div class="col-md-10">
            <?php
            $id_kat = $_GET['id'];
            $query = mysqli_query($con, "SELECT * FROM tbl_kategori WHERE id_kategori = '$id_kat'");
            $ambil = $query->fetch_assoc();
            ?>
            <h4 class="text-center font-weight-bold m-4" style="text-transform: uppercase;">PARFUM <?= $ambil['nama_kategori'] ?></h4>
            <div class="row mx-auto mb-2">
                <?php
                $id_kat = $_GET['id'];
                $ambil = mysqli_query($con, "SELECT tbl_galeri.*, tbl_produk.* FROM tbl_galeri 
                        INNER JOIN tbl_produk ON tbl_galeri.id_produk = tbl_produk.id_produk WHERE id_kategori = '$id_kat'"); ?>
                <?php while ($data = $ambil->fetch_assoc()) { ?>
